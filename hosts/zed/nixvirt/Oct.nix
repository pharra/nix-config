...
in {
  ...
  virtualisation.libvirtd.hooks.qemu."10-Oct" = pkgs.writeShellScript "oct-qemu-hook" ''
    ...
    # === 新增：cgroup v2 isolated partition（特定於 VM "Oct"）===
    setup_isolated_partition() {
      local vcpus="8-15,24-31"   # VM 使用 CCD0
      local hcpus="0-7,16-23"  # 主机剩余 CCD1
      local cgroup_path="/sys/fs/cgroup/oct-isolated"   # 特定於此 VM 的隔离路径
    ...
    restore_isolated_partition() {
      local cgroup_path="/sys/fs/cgroup/oct-isolated"
    ...
    if [ "$machine" == "Oct" ]; then
      ...
      if [ "$command" == "prepare" ]; then
        log "Oct: preparing - GPU detach + conditional hugepages + CPU mask + EPP tuning"

        # 1. GPU detach
        log "Oct: detaching PCI device pci_0000_03_00_1"
        ...
        log "Oct: GPU detached successfully"
        ...
        # 4. EPP 調整
        VM_CPUS="8-15,24-31"
        log "Oct: setting EPP=performance on pinned CPUs ($VM_CPUS)"
        ...
        if [ -f "/sys/devices/system/cpu/cpu8/cpufreq/energy_performance_preference" ]; then
          ...
          log "Oct: verification - cpu8 EPP now: $sample"
        fi

      elif [ "$command" == "started" ]; then
        log "Oct: started - moving QEMU PID to per-VM isolated cgroup"
        ...
        log "Oct: release - restoring environment"

        # 恢復 per-VM isolated partition
        restore_isolated_partition

        VM_CPUS="8-15,24-31"
        log "Oct: restoring EPP=balance_performance on former pinned CPUs ($VM_CPUS)"
        set_epp "$VM_CPUS" "balance_performance"

        log "Oct: reattaching PCI device + hugepages cleanup"
        ...
        log "Oct: GPU reattached and hugepages cleaned up"

        log "Oct: cleanup completed"

      else
        log "Oct: unhandled command=$command"
      fi
