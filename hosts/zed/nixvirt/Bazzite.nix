{
  ...
  Bazzite = linux_template {
    ...
    nvram_path = /fluent/RAMPool/Bazzite.fd;
    no_graphics = true;
  };
in {
  ...
  virtualisation.libvirtd.hooks.qemu."10-Bazzite" = pkgs.writeShellScript "bazzite-qemu-hook" ''
    machine=$1
    command=$2
    log_tag="libvirt-hook-Bazzite"
    ...
    # === 新增：cgroup v2 isolated partition（特定於 VM "Bazzite"）===
    setup_isolated_partition() {
      local vcpus="8-15,24-31"   # VM 使用 CCD0
      local cgroup_path="/sys/fs/cgroup/bazzite-isolated"   # 特定於此 VM 的隔离路径
    ...
    if [ "$machine" == "Bazzite" ]; then

      if [ "$command" == "prepare" ]; then
        log "Bazzite: preparing - GPU detach + conditional hugepages + CPU mask + EPP tuning"

        # 1. GPU detach
        log "Bazzite: detaching PCI device pci_0000_03_00_1"
        ...
        log "Bazzite: GPU detached successfully"
        ...
        # 4. EPP 調整
        VM_CPUS="8-15,24-31"
        log "Bazzite: setting EPP=performance on pinned CPUs ($VM_CPUS)"
        ...
        if [ -f "/sys/devices/system/cpu/cpu8/cpufreq/energy_performance_preference" ]; then
          ...
          log "Bazzite: verification - cpu8 EPP now: $sample"
        fi

      elif [ "$command" == "started" ]; then
        log "Bazzite: started - moving QEMU PID to per-VM isolated cgroup"
        ...
        log "Bazzite: release - restoring environment"
        ...
        VM_CPUS="8-15,24-31"
        log "Bazzite: restoring EPP=balance_performance on former pinned CPUs ($VM_CPUS)"
        set_epp "$VM_CPUS" "balance_performance"

        log "Bazzite: reattaching PCI device + hugepages cleanup"
        ...
        log "Bazzite: GPU reattached and hugepages cleaned up"

        log "Bazzite: cleanup completed"

      else
        log "Bazzite: unhandled command=$command"
      fi
