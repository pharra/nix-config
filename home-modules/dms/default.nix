  ...
  options = {
    home.pharra.dms = {
      ...
      compositor = mkOption {
        ...
        default = attrByPath ["services" "pharra" "dms" "compositor"] "niri" osConfig;
  ...
  config = mkIf cfg.enable {
    ...
    home.activation.installDmsAndCompositorDefaults = lib.hm.dag.entryAfter ["writeBoundary"] ''
      ...
      ${optionalString (cfg.compositor == "hyprland") ''
        copy_if_missing "${defaultHyprlandConfig}/hyprland.conf" "${config.xdg.configHome}/hypr/hyprland.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/binds.conf" "${config.xdg.configHome}/hypr/dms/binds.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/colors.conf" "${config.xdg.configHome}/hypr/dms/colors.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/layout.conf" "${config.xdg.configHome}/hypr/dms/layout.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/monitors.conf" "${config.xdg.configHome}/hypr/dms/monitors.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/windowrules.conf" "${config.xdg.configHome}/hypr/dms/windowrules.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/cursor.conf" "${config.xdg.configHome}/hypr/dms/cursor.conf"
        copy_if_missing "${defaultHyprlandConfig}/dms/outputs.conf" "${config.xdg.configHome}/hypr/dms/outputs.conf"
      ''}
